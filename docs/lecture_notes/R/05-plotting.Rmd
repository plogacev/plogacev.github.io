...
set.seed(42)
knitr::opts_chunk$set(cache.extra = knitr::rand_seed, class.output="r-output", fig.height = 3)
...
```{r, error=T}
ggplot(df, aes(a, a_sq, group = my_group)) + geom_point()  + geom_line() + facet_wrap(~my_group)
```


<!--

I rewrote this part. I could not understand why we have the colorless plot later. Here's my suggestion:
## Grouping and facets

- Color, fill, etc. implicitly group the data set into different subgroups.
- These elements help us to read the data easier. 
- Check our grouped data.

```{r, error=T}
ggplot(df, aes(a, a_sq, group = my_group)) + geom_point()  + geom_line()
```

- See? Without colors, it is extremely hard to see which line is which. 
- Let's take a look at it with colors.

```{r, error=T}
ggplot(df, aes(a, a_sq, color = my_group)) + geom_point()  + geom_line()
```

- Even though this is slightly better, we can do better. 
- We can group our data *explicitly* as well.
- So, let's separate it into different **facets** (aka *'panels'*). 
- We can introduce our new facets with the function `facet_wrap()`. Keep in mind that the grouping variable is introduced with `~`.
