...
In this notebook, we tackle a common data problem in analyzing retail data: Realized sales don't always reflect the actual demand for a product.
This is because when a product goes out of stock, the realized sales are artificially forced to zero, while the latent demand remains what it would have been.
...
The key to a good stockout detection algorithm is that we need to posit stockouts where zeroes occur rather unexpectedly. 
This can happen when sales suddenly drop to zero
