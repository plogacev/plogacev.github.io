functions {
  ...
  real compute_marginal_loglik_path(int t1, int t2, int n_time_points, 
                                    //vector lp_cp, vector lp_ncp,
                                    real prior_lp_cp, real prior_lp_ncp) // , vector lp_cp_magnitude_prior, vector lp_ncp_magnitude_prior
  {
      real lp_path = 0.0;
      ...
      if ( t2 < n_time_points ) { // terminates with a changepoint at the end of the segment
          //lp_path += lp_cp[t2];
          lp_path += prior_lp_cp; // prior: this is a cp
      ...
      if ( t1 < t2 ) {  // aggregate all the non-changes before the changepoint 
          ...
          lp_path += (t2 - t1) * prior_lp_ncp; // prior: all these are not cp
  ...
  real compute_marginal_loglik(//vector lp_cp, vector lp_ncp, 
                               matrix segments_loglik,
                               real prior_lp_cp, real prior_lp_ncp) // , vector lp_cp_magnitude_prior, vector lp_ncp_magnitude_prior
  {
        ...
        for (t2 in 1:n_time_points) {
            ...
            for (t1 in 1:t2) {
                ...
                real cur_segment_loglik = compute_marginal_loglik_path(t1, t2, n_time_points, //lp_cp, lp_ncp,
                                                                        prior_lp_cp, prior_lp_ncp); // , lp_cp_magnitude_prior, lp_ncp_magnitude_prior
...
parameters {
  ...
  // real<lower=2> mean_cp_interval;
  
  real<lower=0, upper=1> cp_prob;
}
...
model {
/*
  //mean_cp_interval ~ lognormal(log(100), 0.2); // normal(350, 10); // assume that the typical time between cps is around 250-450 days on average
  mean_cp_interval ~ normal(100, 10); // normal(350, 10); // assume that the typical time between cps is around 250-450 days on average
  real prior_lp_cp = log(1/mean_cp_interval);
  real prior_lp_ncp = log(1-1/mean_cp_interval);
*/

  cp_prob ~ beta(1, 100);
  real prior_lp_cp = log(cp_prob);
  real prior_lp_ncp = log1m(cp_prob);

  target += compute_marginal_loglik(segments_loglik, prior_lp_cp, prior_lp_ncp); // lp_cp, lp_ncp,  //, lp_cp_magnitude_prior, lp_ncp_magnitude_prior
}
